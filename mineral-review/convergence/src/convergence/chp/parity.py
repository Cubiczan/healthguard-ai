...
from convergence.chp.models import ModelParityCheck, ModelTier
...
def _infer_tier(model_name: str) -> ModelTier:
    ...
    if any(token in name for token in ("gpt-5", "claude 4", "claude-4", "high", "llama3.3-70b", "deepseek")):
        return ModelTier.HIGH
    if any(token in name for token in ("sonnet", "4o", "mid", "gpt-4")):
        return ModelTier.MID
    if any(token in name for token in ("mini", "small", "haiku", "gpt-oss-20b")):
