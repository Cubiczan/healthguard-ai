...
from convergence.chp.models import SessionStatus, Verdict
...
def evaluate_r0_gate(*, solvable: bool, scoped: bool, valid: bool, worth_it: bool) -> GateEvaluation:
    """R0 gate: all four checks must PASS or session halts."""
    results = {
...
def evaluate_phase_gate(round_number: int, phase_one_status: SessionStatus) -> Verdict:
    """Phase gate: cannot enter IMPLEMENTATION (round >= 3) without SPEC lock."""
    if round_number <= 2:
        return Verdict.PASS
